...
여기 있는 문장을 그대로 외우려고 하지 않는다. 면접에서는 문서처럼 말하면 오히려 내가 한 일 같지 않고, 중간에 한 단어가 생각 안 나면 답변 전체가 막히기 쉽다.

그래서 답변은 항상 **세 덩어리만 기억한다.**

1. **뭐가 문제였는지**
2. **그래서 내가 뭘 선택하거나 바꿨는지**
3. **그 결과 뭘 확인했는지**

기술 용어는 그 세 덩어리를 설명할 때만 꺼낸다. 아래의 `면접에서 이렇게 말하기`는 암기 문장이 아니라, 소리 내서 두세 번 말해 보고 내 말로 조금씩 바꾸기 위한 초안이다.

### 프로젝트를 한 번에 설명하면

**한 줄로:** CCTV 영상을 실시간으로 보고 사람이 쓰러지는 상황을 잡아서 관제 화면까지 알려주는 시스템이다.

**면접에서 이렇게 말하기:**

> "처음에는 AI 모델 성능만 잘 나오면 된다고 생각하기 쉬운데, 실제 관제에서는 그 뒤가 더 중요했습니다. 어떤 카메라의 어떤 프레임에서 이벤트가 났는지, 그 정보가 MQTT와 백엔드를 거쳐 화면의 박스까지 같은 기준으로 이어져야 했습니다. 그래서 저는 모델뿐 아니라 RTSP 입력부터 tracking, 이벤트 payload, 화면 overlay까지 한 흐름으로 보면서 문제를 잡았습니다."

**기억할 단어 3개:** `실시간`, `증거 연결`, `끝까지 추적`

### 내 강점을 한 번에 설명하면

**한 줄로:** 화면에 이상한 게 보이면 모델 탓부터 하지 않고, 어느 단계에서 틀어졌는지 쪼개서 보는 편이다.

**면접에서 이렇게 말하기:**

> "제가 이 프로젝트에서 가장 많이 한 일은 문제를 단계별로 쪼개는 일이었습니다. 예를 들어 박스가 늦게 따라오면 AI가 느린 건지, 백엔드 전달이 늦은 건지, 프론트가 오래된 값을 고른 건지를 따로 봤습니다. 그래서 감으로 threshold만 만지는 것보다 로그와 frameId, timestamp 같은 근거를 먼저 확인하는 습관이 생겼습니다."
...
- "그래서 ~~로 나눠서 봤습니다."
- "제가 직접 확인한 건 ~~까지입니다."
- 아직 검증이 덜 됐으면 "여기까지는 확인했고, ~~는 추가 검증이 필요합니다"라고 끝낸다.
...
### 6.1 SK쉴더스 조별면접 6문항 압축

1. **1분 자기소개**  
   의공학을 전공하며 AI와 딥러닝을 공부했고, 모델 자체보다 결과가 실제 서비스에서 쓰이는 과정에 흥미를 느껴 개발 역량을 보완했습니다. 최종 프로젝트에서는 5인 팀의 팀장 겸 AI 담당으로 스마트 안전관제 시스템을 개발했고, AI 결과를 MQTT로 백엔드와 관제 화면까지 연결했습니다. 여러 기술을 연결해 실제로 쓸 수 있는 시스템을 만드는 과정이 제 강점이라고 생각합니다.

2. **최종 프로젝트와 본인의 역할**  
   CCTV 영상에서 낙상이나 이상행동을 탐지해 관제 화면에 알림을 주는 시스템을 개발했습니다. Pose와 LSTM 기반 행동분석, Tracking과 실시간 처리 개선을 맡았고, MQTT 연동과 파트별 데이터 규격을 맞추는 통합 작업도 진행했습니다.

3. **가장 어려웠던 문제와 해결 방법**  
   행동 탐지 결과가 불안정한 문제가 있었습니다. 모델 정확도만의 문제라고 단정하지 않고, LSTM이 한 사람의 연속 데이터를 필요로 한다는 점에서 Tracking 끊김을 원인으로 의심했습니다. Pose → Tracking ID → LSTM 입력을 단계별로 확인해 끊기는 구간을 찾았고, 누락 프레임 처리와 Tracking 로직을 개선해 결과를 안정화했습니다.

4. **팀 프로젝트에서 갈등이 생기면?**  
   누가 맞는지를 따지기보다 프로젝트 목표와 실제 결과를 기준으로 판단하려고 합니다. 파트별로 필요한 데이터 형태가 달랐을 때도 각자의 입력과 출력을 정리하고 공통 규격을 맞추는 방식으로 해결했습니다.

5. **고객이 무리한 요구를 하면?**  
   바로 안 된다고 하기보다 요구의 목적부터 확인하겠습니다. 일정, 기술적 제약, 기존 시스템에 미치는 영향을 검토해 가능한 대안을 제시하고, 품질이나 일정에 영향이 있다면 이를 설명한 뒤 우선순위를 조율하겠습니다.

6. **왜 SK쉴더스이며, 어떤 개발자가 되고 싶은가?**  
   스마트 안전관제 시스템을 개발하며 기술이 실제 현장에서 사용되는 과정에 흥미를 느꼈습니다. 모델 성능뿐 아니라 운영 환경에서의 안정성과 시스템 간 연결이 중요하다는 것을 경험했고, 입사 후에는 팀의 시스템과 개발 방식을 빠르게 익혀 실제 운영에 도움이 되는 개발자로 성장하고 싶습니다.
