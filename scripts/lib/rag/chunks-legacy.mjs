...
/**
 * Legacy section/block chunking (preserved baseline).
 * chunkSchemaVersion: legacy-v1
 */
export function buildRagIndexLegacy(documents, options = {}) {
  ...
  for (const document of orderedDocuments) {
    ...
    for (const section of splitSections(document.body)) {
      for (const chunk of chunkSection(section, chunkSize)) {
        const built = makeChunk(document, chunk.section, chunk.text, chunkOrder++);
        built.chunkSchemaVersion = 'legacy-v1';
...
/** @deprecated use buildRagIndexLegacy — kept for import compatibility */
